...
public class ProjectsController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<string>>> Approve(int id)
    {
        ...
        if (!result)
            return NotFound(ApiResponse<string>.ErrorResponse($"Project with ID {id} not found"));

        return Ok(ApiResponse<string>.SuccessResponse(null, "Project approved and published"));
    }
}
