...
public class InvoicesController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<bool>>> UpdateStatus(int id, [FromBody] InvoiceUpdateStatusDto statusDto)
    {
        ...
        if (!success)
            return NotFound(ApiResponse<bool>.ErrorResponse($"Invoice with ID {id} not found"));
    ...
    /// <summary>
    /// Gets a paginated list of invoices with filters and search.
    /// </summary>
    [HttpGet("paged")]
    public async Task<ActionResult<ApiResponse<PagedResult<InvoiceResponseDto>>>> GetAll([FromQuery] InvoiceQueryParams queryParams)
    {
